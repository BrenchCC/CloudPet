...
from PyQt5.QtWidgets import QApplication, QWidget, QMenu, QAction, QLabel, QGraphicsDropShadowEffect, QFileDialog, \
    QDialog, QVBoxLayout, \
...
class DesktopPet(QWidget, QObject):
    ...
    def init_ui(self):
        ...
        self.menu.addActions(
            [chat_action, web_action, change_icon_action, change_nickname_action, settings_action, exit_action])
    ...
    def change_icon(self):
        ...
        new_icon_path, _ = QFileDialog.getOpenFileName(self, "选择新图标", "",
    ...
    def show_settings_dialog(self):
        ...
        settings_dialog.move(screen_center.x() - settings_dialog.width() // 2,
                             screen_center.y() - settings_dialog.height() // 2)
        ...
        ok_button.clicked.connect(
            lambda: self.save_all_config(self.walk_checkbox.isChecked(), self.random_question_checkbox.isChecked(),
                                         self.openai_key_input.text(),
                                         self.chat_window_shortcut_input.keySequence().toString(),
                                         self.chat_web_shortcut_input.keySequence().toString()))
    ...
    def change_size(self):
        ...
        screen_geometry = QApplication.desktop().availableGeometry()
        screen_center = screen_geometry.center()
        width_input_dialog.move(screen_center.x() - width_input_dialog.width() // 2,
                                screen_center.y() - width_input_dialog.height() // 2)
        ...
        height_input_dialog.move(screen_center.x() - height_input_dialog.width() // 2,
                                 screen_center.y() - height_input_dialog.height() // 2)
